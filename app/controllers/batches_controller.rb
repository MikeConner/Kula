class BatchesController < ApplicationController
  ...
  def export
    ...
    csv_data = "Type,amount,status,date,month,year,payment_id,check_num,payment_method,confirmation,cause_id,org_name,org_email,org_phone,org_fax,address1,address2,address3,city,state,zipcode,school?,international?,has_ach?,comment\n"
    payments.each do |payment|
      csv_data += "Payment,#{payment.amount},#{payment.status},#{payment.date.try(:strftime, ApplicationHelper::CSV_DATE_FORMAT)},"
      cause = payment.cause
      csv_data += "#{payment.month},#{payment.year},#{payment.id},#{payment.check_num},#{payment.payment_method},#{payment.confirmation},"
      ...
      csv_data += "#{csv_sanitize(cause.address2)},#{csv_sanitize(cause.address3)},#{csv_sanitize(cause.city)},#{csv_sanitize(cause.region)},#{cause.postal_code},#{cause.school?}, #{cause.international?},#{cause.has_eft_bank_info?},#{csv_sanitize(payment.comment)}\n"
    end
    ...
    adjustments.each do |adjustment|
      ...
      csv_data += "#{cause.cause_identifier},#{csv_sanitize(cause.org_name)},#{cause.org_email},#{cause.org_phone},#{cause.org_fax},#{csv_sanitize(cause.address1)},"
      csv_data += "#{csv_sanitize(cause.address2)},#{csv_sanitize(cause.address3)},#{csv_sanitize(cause.city)},#{csv_sanitize(cause.region)},#{cause.postal_code},#{cause.school?}, #{cause.international?},#{cause.has_eft_bank_info?},#{csv_sanitize(adjustment.comment)}\n"
    end
